...
void mpate311_shabib9() {
  printf("Taha Habibullah and Manil Patel\n");
  printf("What we learned is that git is a very powerful tool that allows users to easily              collaborate with each other on projects. One thing that we struggled with at the             beginning of figuring out git was that, at first, the repository wouldn't clone              but we later found out it's because we just didn't include https:\\ in the repo              url which caused problems.\n");
}
